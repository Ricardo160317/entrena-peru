...
export default function Progreso({ entrenamientos, medidas, onMedidaGuardada }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {sinNadaAun && (
        <div className="bg-[#171D1B] border border-[#262E2B] rounded-xl shadow-[0_4px_16px_rgba(0,0,0,0.06)] p-6 text-center">
      ...
      {ultimaMedida && (
        <div>
          <p className="text-xs text-[#8B948F] mb-2">Última medición de tu balanza</p>
          <div className="bg-[#171D1B] border border-[#262E2B] rounded-xl shadow-[0_4px_16px_rgba(0,0,0,0.06)] p-4 grid grid-cols-4 gap-2 text-center">
          ...
          <p className="text-[10px] text-[#5F6864] mt-1.5 text-right">{fechaLegible(String(ultimaMedida.fecha).slice(0, 10))}</p>
        </div>
      )}

      {datosComposicion.length > 1 && (
        <div>
          <p className="text-xs text-[#8B948F] mb-2">Evolución de tu composición corporal</p>
          <div className="bg-[#171D1B] border border-[#262E2B] rounded-xl shadow-[0_4px_16px_rgba(0,0,0,0.06)] p-4">
            <ResponsiveContainer width="100%" height={180}>
              <LineChart data={datosComposicion} margin={{ top: 5, right: 8, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#262E2B" />
                <XAxis dataKey="fechaCorta" tick={{ fill: "#5F6864", fontSize: 10 }} axisLine={{ stroke: "#262E2B" }} tickLine={false} />
                <YAxis tick={{ fill: "#5F6864", fontSize: 10 }} axisLine={false} tickLine={false} width={30} />
                <Tooltip contentStyle={tooltipStyle} />
                <Line type="monotone" dataKey="grasa" name="Grasa %" stroke="#F59E0B" strokeWidth={2} dot={{ r: 3 }} connectNulls />
                <Line type="monotone" dataKey="musculo" name="Músculo %" stroke="#C8FF3D" strokeWidth={2} dot={{ r: 3 }} connectNulls />
                <Line type="monotone" dataKey="agua" name="Agua %" stroke="#C8FF3D" strokeWidth={2} dot={{ r: 3 }} connectNulls />
              </LineChart>
            </ResponsiveContainer>
            <div className="flex gap-4 justify-center mt-1">
              <Leyenda color="#F59E0B" label="Grasa" />
              <Leyenda color="#C8FF3D" label="Músculo" />
              <Leyenda color="#C8FF3D" label="Agua" />
            </div>
          </div>
        </div>
      )}
      ...
      {sesionesPorSemana.length > 0 && (
        <div>
          <p className="text-xs text-[#8B948F] mb-2">Frecuencia de entrenamiento (últimas semanas)</p>
          <div className="bg-[#171D1B] border border-[#262E2B] rounded-xl shadow-[0_4px_16px_rgba(0,0,0,0.06)] p-4">
            <ResponsiveContainer width="100%" height={140}>
              <BarChart data={sesionesPorSemana} margin={{ top: 5, right: 8, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#262E2B" vertical={false} />
                <XAxis dataKey="semana" tick={{ fill: "#5F6864", fontSize: 10 }} axisLine={{ stroke: "#262E2B" }} tickLine={false} />
                <YAxis allowDecimals={false} tick={{ fill: "#5F6864", fontSize: 10 }} axisLine={false} tickLine={false} width={24} />
                <Tooltip contentStyle={tooltipStyle} />
                <Bar dataKey="total" name="Sesiones" fill="#C8FF3D" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      {entrenamientos.length > 0 && (
        <>
          <div>
            <p className="text-xs text-[#8B948F] mb-2">Sesiones recientes</p>
            <div className="space-y-2">
              {[...entrenamientos].reverse().slice(0, 6).map((s, i) => (
                <div key={i} className="bg-[#171D1B] border border-[#262E2B] rounded-xl shadow-[0_4px_16px_rgba(0,0,0,0.06)] p-3 flex items-center justify-between">
                  <div>
          ...
          <div>
            <p className="text-xs text-[#8B948F] mb-2">Evolución de un ejercicio</p>
            <select
              value={seleccionado || ""}
              onChange={(e) => setSeleccionado(e.target.value || null)}
              className="w-full bg-[#171D1B] border border-[#262E2B] rounded-lg px-3 py-2.5 text-sm outline-none"
            >
            ...
            {seleccionado && historialEjercicio.length > 0 && (
              <div className="mt-3 bg-[#171D1B] border border-[#262E2B] rounded-xl shadow-[0_4px_16px_rgba(0,0,0,0.06)] p-4">
                <ResponsiveContainer width="100%" height={180}>
                  <LineChart data={historialEjercicio} margin={{ top: 5, right: 8, left: -20, bottom: 0 }}>
                    ...
                    <Line type="monotone" dataKey="peso" stroke="#C8FF3D" strokeWidth={2.5} dot={{ r: 4, fill: "#C8FF3D" }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            )}
...
function MiniStat({ label, valor, sufijo }) {
  return (
    <div>
      ...
      <p className="text-[10px] text-[#5F6864]">{label}</p>
    </div>
  );
}
...
function Leyenda({ color, label }) {
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className="text-[10px] text-[#8B948F]">{label}</span>
    </div>
  );
}
